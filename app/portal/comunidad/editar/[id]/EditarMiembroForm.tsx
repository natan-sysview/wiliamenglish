...
import { ArrowLeft, Save, User, Mail, Phone, Shield, MapPin, Monitor, Loader2, Pencil } from "lucide-react";

export default function EditarMiembroForm({ usuario }: { usuario: any }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  
  // Pre-cargar los datos del usuario de la BD
  const [formData, setFormData] = useState({
    nombre: usuario.nombre || "",
    email: usuario.email || "",
    telefono: usuario.telefono || "",
    rol: usuario.rol || "ALUMNO",
    sucursal: usuario.sucursal || "QUERETARO",
    modalidad: usuario.modalidad || "PRESENCIAL",
  });
    ...
    <div className="max-w-4xl mx-auto px-6 lg:px-8 py-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-10">
        <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight flex items-center gap-3">
          <Pencil className="w-8 h-8 text-[#2952F5]" />
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#2952F5] to-[#CC0000]">
            Modificar miembro
          </span>
        </h1>
        <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">
          Estás editando la información de <strong>{usuario.nombre}</strong>.
        </p>
      </div>
