...
export default async function PortalPage() {
  ...
  // Permisos para ver el módulo de Analítica
  const isAdminOrStaff = session?.user?.rol === "ADMIN" || session?.user?.rol === "STAFF";

  return (
    <>
      {/* Tarjeta Hero de Bienvenida Expansiva */}
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-white/50 dark:border-slate-800 shadow-2xl shadow-[#2952F5]/5 dark:shadow-none rounded-[2.5rem] p-10 md:p-14 mb-12 overflow-hidden relative transition-colors duration-300">
      ...
      <div className="flex items-center justify-between mb-8">
        <h3 className="text-2xl lg:text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight">Módulos Operativos</h3>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 pb-12">
        ...
        {/* Solo mostrar Analítica a ADMIN y STAFF */}
        {isAdminOrStaff && (
          <Link href="/portal/analitica" className="group block h-full">
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/60 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none h-full p-10 lg:p-12 rounded-[2.5rem] relative overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-[#2952F5]/20 hover:-translate-y-2">
              <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-[#2952F5]/20 dark:from-blue-900/30 to-transparent rounded-bl-full -z-10 transition-transform duration-700 group-hover:scale-125"></div>
              
              <div className="w-20 h-20 bg-white dark:bg-slate-800 rounded-3xl flex items-center justify-center shadow-md shadow-blue-100/50 dark:shadow-blue-900/20 text-[#2952F5] dark:text-blue-400 mb-8 transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-3">
                <BarChart3 size={40} strokeWidth={1.5} />
              </div>
              
              <h3 className="text-3xl font-extrabold text-slate-800 dark:text-slate-100 mb-5 group-hover:text-[#2952F5] dark:group-hover:text-blue-400 transition-colors">Analíticas (B.I.)</h3>
              <p className="text-lg lg:text-xl text-slate-500 dark:text-slate-400 leading-relaxed mb-10 font-light">
                Visualiza el pulso de la escuela con gráficos en tiempo real, KPIs y distribución demográfica.
              </p>
              
              <div className="flex items-center text-[#2952F5] dark:text-blue-400 font-bold text-lg lg:text-xl group-hover:translate-x-3 transition-transform">
                Ver Métricas <span className="ml-2 text-3xl">&rarr;</span>
              </div>
            </div>
          </Link>
        )}
